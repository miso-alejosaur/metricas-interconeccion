...
public class ArregloDinamico<T extends Comparable<T>> implements ILista<T> {
	...
	private void insertInMiddle(T elemento, int pos) {
		if (tamanoAct == tamanoMax) {
			tamanoMax = 2 * tamanoMax;
		}
		T[] copia = elementos;
	...
	public void insertElement(T elemento, int pos) throws PosException, NullException {
		...
		makeInsertion(elemento, pos);

		tamanoAct++;

	}

	private void makeInsertion(T elemento, int pos) {
		if (pos == 1) {
			addFirst(elemento);
		} else if (tamanoAct + 1 == pos) {
			addLast(elemento);
		} else {
			insertInMiddle(elemento, pos);
		}
	}
	...
	private void removeInMiddle(int pos) {
		T[] copia = elementos;
		elementos = (T[]) new Comparable[tamanoMax];
	...
	private void performDeletion(int pos) {
		if (pos == 1) {
			removeFirst();
